  ...
  playbackDetails(selectedMedia, type, session) {
    ...
    return (
      <>
        ...
        {selectedMedia.Part[0].Stream[0].decision === "transcode" || selectedMedia.Part[0].Stream[0].decision === "copy" ? (
          <>
        ...
        {selectedMedia.Part[0].Stream[1].decision === "transcode" ? (
          <p>
            ...
            {this.formatDecision(selectedMedia.Part[0].Stream[1].decision)} {selectedMedia.Part[0].Stream[1].displayTitle}
            <ArrowIcon />
        ...
        <p>
          ...
          <span className={"session--secure secure-" + session.Player.secure}>{session.Player.secure ? <LockIcon /> : <UnlockIcon />}</span>
        </p>
      </>
    );
  }
        ...
        return (
          <div className="sessions">
            {sessions.map((session) => {
              ...
              let playback = selectedMedia.Part ? selectedMedia.Part[0].decision : "unknown";
              switch (session.type) {
                case "episode":
              ...
              return (
                <div className="session" data-id={session.Session ? session.Session.id : "pending"} key={session.Player.machineIdentifier + session.ratingKey + "__session"}>
                  <SessionMedia
                    id={media}
                    ...
                    progress={session.Media.length > 0 && session.viewOffset ? (session.viewOffset / session.Media[0].duration) * 100 : 0}
                    playbackState={session.Player.state}
                  />
                  <div className="session--info">
                    <p>User: {session.User.title}</p>
                    <p className="session--title">{session.title + (session.live ? " (Live)" : "")}</p>
                    {session.type === "episode" ? <p className="session--show">{session.grandparentTitle + (session.parentTitle ? " | " + session.parentTitle : "")}</p> : null}
