...
const logger = loggerMain.core.child({ label: 'mail.mailer' });
  ...
  mail(subject, title, text, img, to = [], name = []) {
    ...
    try {
      ...
      to.forEach((send, i) => {
        ...
        i++;
        setTimeout(async () => {
          logger.debug(`MAILER: Sending email from: ${config.get('email.from')} to ${send} with the subject ${subject}`);
          try {
            this.transport.sendMail({
              ...
              text,
              onError: (e: string) => {
                logger.error(`MAILER: Message failed to send`, { message: e });
              },
              onSuccess: (s: { messageId: any; }) =>
                logger.debug('MAILER: Message sent: %s', { messageId: s.messageId }),
            });
          } catch (err) {
            logger.error(`MAILER: Message failed to send`, err);
          }
        }, timeout);
      });
    } catch (err) {
      logger.error('MAILER: Email failed', err);
    }
  }
}
